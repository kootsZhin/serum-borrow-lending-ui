  ...
  return (
    ...
    >
      {value === index && (
        <Box sx={{ p: 3 }}>
          <Stack spacing={2}>
            <TextField id="outlined-basic" label={`Enter ${asset} Amount`} variant="outlined" onChange={(event) => { setDepositAmount(Number(event.target.value)) }} />
            <Button disabled={!publicKey} variant="contained" onClick={onClick}>
              {publicKey ? `Deposit ${asset}` : "Connect Wallet"}
            </Button>
          </Stack>
        </Box>
      )}
